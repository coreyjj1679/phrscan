...
export function ResultPanel({ result, error }: Props) {
  ...
  const human = error ? humanizeError(error) : null;
    ...
    <div className="space-y-2 rounded bg-gray-900/50 p-3 sm:p-4">
      ...
      {error && (
        <div className="rounded bg-red-950/50 p-3 ring-1 ring-red-800">
          ...
          {human && error.trim() !== human && (
            <details className="mt-1.5">
              <summary className="cursor-pointer text-xs text-red-500/70 hover:text-red-400">
                raw error
              </summary>
              <pre className="mt-1 max-h-32 overflow-auto whitespace-pre-wrap break-all font-mono text-xs text-red-500/60">
                {error}
              </pre>
            </details>
          )}
          ...
          {isVoid(result.decoded) ? (
            ...
            <div className="rounded bg-gray-900 p-3 ring-1 ring-gray-700">
              <div className="mb-1 flex items-center justify-between gap-2">
                <p className="text-xs text-gray-400">Decoded</p>
                <CopyButton text={formatValue(result.decoded)} label="copy" />
              </div>
      ...
      {logs.map((log, i) => (
        <details key={i} className="group rounded bg-gray-900 ring-1 ring-gray-700">
          <summary className="flex cursor-pointer items-center gap-2 px-3 py-2">
            <span className="rounded bg-violet-900/50 px-1.5 py-0.5 font-mono text-xs font-semibold text-violet-400">
              {log.eventName}
            </span>
                ...
                {log.raw.topics.length > 0 && (
                  <div className="text-xs">
                    ...
                    {log.raw.topics.map((t, ti) => (
                      <span key={ti} className="block break-all pl-4 font-mono text-xs text-gray-500">
                        [{ti}] {t}
                      </span>
                    ))}
                  </div>
